...
def make_features(batch, side, data_type='text'):
    ...
    Returns:
        ...
        of size (len x batch).
    """
    assert side in ['src', 'tgt','template']
...
def build_dataset(fields, data_type, src_data_iter=None, src_path=None,
                  src_dir=None, tgt_data_iter=None, tgt_path=None,template_iter=None,template_path=None,
    ...
    template_examples_iter,num_tmp_feats= \
        TextDataset.make_text_examples_nfeats_tpl(
            template_iter, template_path, tgt_seq_length_trunc, "template")


    if data_type == 'text':
        dataset = TextDataset(fields, src_examples_iter, tgt_examples_iter,template_examples_iter,
...
def build_vocab(train_dataset_files, fields, data_type, share_vocab,
                ...
                tgt_vocab_path, tgt_vocab_size, tgt_words_min_frequency,template_vocab_path):
    """
    Args:
    ...
    tp_vocab=load_vocabulary(template_vocab_path,tag="template")
    ...
    for path in train_dataset_files:
        ...
        for ex in dataset.examples:
            for k in fields:
                ...
                elif k=='template' and tp_vocab:
                    val = [item for item in val if item in tp_vocab]
    ...
    if data_type == 'text':
        ...
        _build_field_vocab(fields["template"], counter["template"],
                           max_size=src_vocab_size,
                           min_freq=src_words_min_frequency)
        logger.info(" * template vocab size: %d." % len(fields["template"].vocab))
        ...
        if share_vocab:
            ...
            fields["template"].vocab=merged_vocab
